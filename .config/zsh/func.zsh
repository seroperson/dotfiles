...
is_arg_present() { [ "x$1" != "x" ] }
# `get_or_else $A $B`
get_or_else() { is_arg_present $1 && echo $1 || echo $2 }

# }}}

# {{{ 'is_*_(available|enabled)' functions

is_base16_shell_available() { is_file_exists $BASE16_SHELL_DIRECTORY }
is_tmux_enabled() { is_command_present tmux }
is_fasd_enabled() { is_command_present fasd }

# }}}

# {{{ 'is_in_*' functions

is_in_x() { is_arg_present $DISPLAY }
is_in_tmux() { is_arg_present $TMUX }
...
enable_base16_shell() {
    ...
    if is_arg_present $theme_name; then
        ...
        if ! is_file_exists $theme_path; then
...
list_base16_shell_themes() { ls $BASE16_SHELL_DIRECTORY | grep ".sh" }
